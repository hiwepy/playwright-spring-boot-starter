...
public class PlaywrightPdfApplication_Test4 implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        try {
            ...
            // 截图测试
            captureScreenshot(UUID.randomUUID().toString(), new PageScreenshotTemp().setUrl("https://www.baidu.com").setIndex(1), null).exceptionally(throwable -> {
                throwable.printStackTrace();
                return null;
            }).join();

            // 生成pdf测试
            pageToPdf(UUID.randomUUID().toString(), new PageScreenshotTemp().setUrl("https://www.baidu.com").setIndex(1)).exceptionally(throwable -> {
                throwable.printStackTrace();
                return null;
            }).join();
        } catch (Exception e) {
