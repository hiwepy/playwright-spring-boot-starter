...
@Slf4j
public class PlaywrightScreenshotApplication_Test3 implements CommandLineRunner {
    ...
    public static void main(String[] args) throws Exception {
        SpringApplication.run(PlaywrightScreenshotApplication_Test3.class, args);
    }
    ...
    public void run(String... args) throws Exception {
        try {
            ...
            // 截图测试
            captureScreenshot(UUID.randomUUID().toString(), new PageScreenshotTemp().setUrl("https://www.baidu.com").setIndex(1), null).whenComplete((pdf, throwable) -> {
            ...
            // 生成pdf测试
            pageToPdf(UUID.randomUUID().toString(), new PageScreenshotTemp().setUrl("https://www.baidu.com").setIndex(1)).whenComplete((pdf, throwable) -> {
                try(ByteArrayInputStream input = new ByteArrayInputStream(pdf.getBuffer());) {
     ...
     * @return
     */
    protected List<PageScreenshotTemp> captureScreenshots(String rendeId, List<PageScreenshotTemp> urlTemps, String selector) {
        log.info("Capturing screenshots for urls: {}", urlTemps.stream().map(PageScreenshotTemp::getUrl).collect(Collectors.toList()));
        // 1、使用CompletableFuture异步处理
        List<CompletableFuture<PageScreenshotTemp>> futureList = urlTemps.stream()
     ...
     * @return
     */
    protected CompletableFuture<PageScreenshotTemp> captureScreenshot(String rendeId, PageScreenshotTemp urlTemp, String selector){
     ...
     * @return
     */
    protected List<PageScreenshotTemp> pageToPdfs(String rendeId, List<PageScreenshotTemp> urlTemps) {
        log.info("Capturing screenshots for urls: {}", urlTemps.stream().map(PageScreenshotTemp::getUrl).collect(Collectors.toList()));
        // 1、使用CompletableFuture异步处理
        List<CompletableFuture<PageScreenshotTemp>> futureList = urlTemps.stream()
    ...
    protected CompletableFuture<PageScreenshotTemp> pageToPdf(String rendeId, PageScreenshotTemp urlTemp) {
        // 1、使用CompletableFuture.supplyAsync()方法，异步执行截图
        return CompletableFuture.supplyAsync(() -> {
